...
The resulting model fit was good with a low out of sample error rate (0.63%).  Out of sample error is an internal error estimate of the random forect as it is being constructed.
...
Additional validation of the model was done using the testing data created from the inital training data.  Although not as good as the out of sample error rate would indicate (which is an expected result), the model is still very good as can be seen in the confusion matrix with an accuracy of 99.1% (95% CI: 98.8% - 99.4%).  
